...
export class DatabaseStorage implements IStorage {
  ...
  private async generateRealCollaborationNetwork(artistName: string): Promise<NetworkData> {
    ...
    // Simplified role detection - use basic types from data sources

    // Main artist is always typed as 'artist' - let OpenAI and MusicBrainz provide role details
    ...
    // Use roles as provided by OpenAI and MusicBrainz - no internal overrides
    ...
    try {
      ...
      // Use only authentic MusicBrainz data - no hardcoded additions
      ...
      // No Wikipedia or hardcoded fallbacks - only use MusicBrainz data as-is
      if (collaborationData.artists.length === 0) {
        console.log(`👤 [DEBUG] No MusicBrainz collaborations found for "${artistName}" - returning main artist only`);
        ...
        // No fallback collaborations - using pure API-only architecture
        console.log(`🚨 [DEBUG] No real collaboration data found for "${artistName}" from APIs - returning main artist only`);
