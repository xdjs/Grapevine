...
var MusicBrainzService, musicBrainzService;
var init_musicbrainz = __esm({
  "server/musicbrainz.ts"() {
    "use strict";
...
var musicnerd_service_exports = {};
__export(musicnerd_service_exports, {
  musicNerdService: () => musicNerdService
});
var MusicNerdService, musicNerdService;
var init_musicnerd_service = __esm({
  "server/musicnerd-service.ts"() {
    "use strict";
    MusicNerdService = class {
      supabase;
      isAvailable = false;
      supabaseUrl = "";
      useRestApi = false;
    ...
    musicNerdService = new MusicNerdService();
  }
});
...
var supabaseUrl, supabaseKey, supabase, connectionString, db;
var init_supabase = __esm({
  "server/supabase.ts"() {
    "use strict";
    supabaseUrl = process.env.SUPABASE_URL || "https://your-project-ref.supabase.co";
    supabaseKey = process.env.SUPABASE_ANON_KEY || "your-anon-or-service-role-key";
...
import { z } from "zod";
var artists, collaborations, insertArtistSchema, insertCollaborationSchema, networkNodeSchema, networkLinkSchema, networkDataSchema;
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    collaborations = pgTable("collaborations", {
      id: serial("id").primaryKey(),
    ...
    insertArtistSchema = createInsertSchema(artists).omit({
      id: true
    });
    insertCollaborationSchema = createInsertSchema(collaborations).omit({
      id: true
    });
    networkNodeSchema = z.object({
      id: z.string(),
      name: z.string(),
    ...
    networkLinkSchema = z.object({
      source: z.string(),
      target: z.string()
    });
    networkDataSchema = z.object({
...
var MemStorage = class {
  ...
  constructor() {
    ...
    this.collaborations = /* @__PURE__ */ new Map();
  ...
  generateCollaboratorNames(artistName) {
    const hash = artistName.split("").reduce((a, b) => a + b.charCodeAt(0), 0);
    const numCollaborators = 2 + hash % 3;
    const collaborators = [];
    for (let i = 0; i < numCollaborators; i++) {
      const isProducer = (hash + i) % 2 === 0;
      const type = isProducer ? "producer" : "songwriter";
      const name = isProducer ? `${artistName} Producer ${i + 1}` : `${artistName} Writer ${i + 1}`;
      collaborators.push({ name, type });
    }
    return collaborators;
  }
  async generateDynamicNetworkWithImages(artistName) {
    const nodes = [];
    const links = [];
    let mainArtistImage = null;
    let mainArtistSpotifyId = null;
    ...
    for (let i = 0; i < collaboratorNames.length; i++) {
      ...
      let collaboratorImage = null;
      let collaboratorSpotifyId = null;
      if (spotifyService.isConfigured() && Math.random() > 0.3) {
        try {
          const spotifyCollaborator = await spotifyService.searchArtist(collaborator.name);
          if (spotifyCollaborator) {
            collaboratorImage = spotifyService.getArtistImageUrl(spotifyCollaborator, "medium");
  ...
  async generateRealCollaborationNetwork(artistName) {
    console.log(`\u{1F680} [DEBUG] STARTING generateRealCollaborationNetwork for "${artistName}"`);
    const nodes = [];
    const links = [];
    try {
      console.log(`\u{1F3B5} [DEBUG] Fetching MusicBrainz collaboration data for "${artistName}"`);
      const collaborationData = await musicBrainzService.getArtistCollaborations(artistName);
      console.log(`\u{1F3B5} [DEBUG] MusicBrainz returned ${collaborationData.artists.length} artist collaborators and ${collaborationData.works.length} works for "${artistName}"`);
      let mainArtistImage = null;
      let mainArtistSpotifyId = null;
      if (spotifyService.isConfigured()) {
        try {
      ...
      console.log(`\u{1F50D} [DEBUG] Looking up MusicNerd artist ID for main artist: "${artistName}"`);
      let mainArtistMusicNerdId = null;
      try {
        mainArtistMusicNerdId = await musicNerdService.getArtistId(artistName);
      ...
      for (const collaborator of enhancedCollaborators) {
        console.log(`\u{1F464} [DEBUG] Processing collaborator: "${collaborator.name}" (type: ${collaborator.type})`);
        let collaboratorImage = null;
        let collaboratorSpotifyId = null;
        if (spotifyService.isConfigured()) {
          try {
        ...
        if (collaborator.type === "producer" || collaborator.type === "songwriter") {
          try {
            ...
            const producerCollaborations = await musicBrainzService.getArtistCollaborations(collaborator.name);
            if (producerCollaborations && producerCollaborations.artists.length > 0) {
              const authenticCollaborators = producerCollaborations.artists.filter((c) => c.name !== collaborator.name).slice(0, 3).map((c) => c.name);
        ...
        console.log(`\u{1F3AF} [DEBUG] Created node for "${collaborator.name}" with collaborations:`, collaboratorNode.collaborations);
        nodes.push(collaboratorNode);
        links.push({
          source: artistName,
          target: collaborator.name
  ...
  async getNetworkData(artistName) {
    ...
    if (demoArtists.includes(artistName)) {
      console.log(`\u{1F3B5} [DEBUG] Using enhanced demo data for "${artistName}" to showcase producer/songwriter networks`);
      const mainArtist = await this.getArtistByName(artistName);
      if (mainArtist) {
        return this.generateEnhancedDemoNetwork(mainArtist);
      }
    }
    console.log(`\u{1F3B5} [DEBUG] Using real collaboration data path for "${artistName}"`);
    return this.generateRealCollaborationNetwork(artistName);
  }
  async generateEnhancedDemoNetwork(mainArtist) {
    const nodes = [];
    const links = [];
    const nodeMap = /* @__PURE__ */ new Map();
    let mainArtistMusicNerdId = null;
    ...
    const collaborations2 = await this.getCollaborationsByArtist(mainArtist.id);
    for (const collaboration of collaborations2) {
        ...
        for (const secondaryCollab of secondaryCollaborations) {
          ...
          if (other && other.name !== mainArtist.name && nodeMap.has(other.name)) {
            const existingLink = links.find(
              (link) => link.source === collaborator.name && link.target === other.name || link.source === other.name && link.target === collaborator.name
            );
            if (!existingLink) {
              links.push({
                source: collaborator.name,
...
async function registerRoutes(app2) {
  app2.get("/api/network/:artistName", async (req, res) => {
    try {
  ...
  app2.get("/api/search", async (req, res) => {
    try {
      ...
      console.log(`\u{1F50D} [Server] Searching for artists: ${query}`);
      const { musicNerdService: musicNerdService2 } = await Promise.resolve().then(() => (init_musicnerd_service(), musicnerd_service_exports));
      const artistOptions = await musicNerdService2.getArtistOptions(query);
      if (!artistOptions || artistOptions.length === 0) {
        console.log(`\u{1F4ED} [Server] No artists found for: ${query}`);
        return res.json([]);
      }
  ...
  app2.get("/api/artist-options/:artistName", async (req, res) => {
    try {
      const artistName = req.params.artistName;
      if (!artistName) {
      ...
      const { musicNerdService: musicNerdService2 } = await Promise.resolve().then(() => (init_musicnerd_service(), musicnerd_service_exports));
      const options = await musicNerdService2.getArtistOptions(artistName);
  ...
  app2.delete("/api/clear-cache/:artistName", async (req, res) => {
    try {
      const artistName = req.params.artistName;
      console.log(`\u{1F5D1}\uFE0F [DEBUG] Clearing cache for "${artistName}"`);
      if ("getArtistByName" in storage && "cacheNetworkData" in storage) {
        ...
        if (artist) {
          if ("webmapdata" in artist) {
            ...
            if (storage instanceof DatabaseStorage2) {
              const connectionString2 = process.env.CONNECTION_STRING;
              if (connectionString2) {
                const { Client } = await import("pg");
                const client = new Client({ connectionString: connectionString2 });
  ...
  app2.get("/api/config", async (req, res) => {
    try {
      ...
      if (!musicNerdBaseUrl) {
        console.error("\u{1F527} [CRITICAL] MUSICNERD_BASE_URL environment variable is not set");
        res.status(500).json({ message: "MusicNerd base URL not configured" });
        return;
      }
      console.log(`\u{1F527} [DEBUG] Config endpoint called, returning musicNerdBaseUrl: ${musicNerdBaseUrl}`);
      res.json({
        musicNerdBaseUrl
      });
    } catch (error) {
