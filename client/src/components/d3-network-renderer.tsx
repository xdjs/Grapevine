...
export default function D3NetworkRenderer({
  ...
  // Create vine decorations instance
  const vineDecorationsRef = useRef<VineDecorations | null>(null);
  ...
  /**
   * Render link elements with vine decorations.
   */
  const renderLinks = (
    networkGroup: d3.Selection<SVGGElement, unknown, null, undefined>,
    links: NetworkLink[]
    ...
    // Add vine decorations to each link
    console.log('🔍 [D3Renderer] Adding vine decorations to links...');
    try {
      linkGroups.each(function(d, index) {
        const linkGroup = d3.select(this);
        
        // Create leaves and grapes using VineDecorations
        if (vineDecorationsRef.current) {
          vineDecorationsRef.current.createLeaves(linkGroup, index);
          vineDecorationsRef.current.createGrapes(linkGroup, index);
        } else {
          console.log('⚠️ [D3Renderer] vineDecorationsRef.current is null, skipping vine decorations');
        }
      });
      console.log('🔍 [D3Renderer] Vine decorations added to links');
    } catch (error) {
      console.error('❌ [D3Renderer] Error adding vine decorations:', error);
    }
  ...
  useEffect(() => {
    ...
    // Initialize vine decorations
    console.log('🔍 [D3Renderer] Initializing vine decorations...');
    try {
      ...
      // Continue without vine decorations if there's an error
      vineDecorationsRef.current = null;
    }
    ...
    simulation.on("tick", () => {
      ...
      try {
        linkElements.each(function(d, linkIndex) {
          ...
          // Update leaves and grapes using VineDecorations
          if (vineDecorationsRef.current) {
            vineDecorationsRef.current.updateLeaves(linkGroup, linkIndex, source, target);
            vineDecorationsRef.current.updateGrapes(linkGroup, linkIndex, source, target);
          }
        });
      } catch (error) {
        console.error('❌ [D3Renderer] Error updating vine decorations:', error);
      }
