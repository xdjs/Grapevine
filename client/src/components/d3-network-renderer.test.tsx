...
describe('D3NetworkRenderer', () => {
  ...
  describe('Leaf Decorations', () => {
    it('should render leaf decorations for each link', () => {
      const mockLinkGroup = {
      ...
      (d3.select as any).mockReturnValue(mockNetworkGroup);

      const dataWithLinks = {
        nodes: [
          { id: 'artist1', name: 'Artist 1', type: 'artist' as const, size: 20, imageUrl: null },
          { id: 'artist2', name: 'Artist 2', type: 'artist' as const, size: 20, imageUrl: null },
        ],
        links: [
          { source: 'artist1', target: 'artist2', type: 'collaboration' as const },
        ],
      };

      const props = createDefaultProps({ data: dataWithLinks });
      render(<D3NetworkRenderer {...props} />);

      // Verify that leaf decorations are rendered
      expect(mockNetworkGroup.selectAll).toHaveBeenCalledWith('.leaf-decoration');
    });

    it('should create left and right leaves for each link', () => {
      const mockLeafGroup = {
        append: vi.fn().mockReturnValue({
          attr: vi.fn().mockReturnThis(),
          style: vi.fn().mockReturnThis(),
        ...
        selectAll: vi.fn().mockReturnValue({
          data: vi.fn().mockReturnValue({
            enter: vi.fn().mockReturnValue({
              append: vi.fn().mockReturnValue({
                each: vi.fn().mockImplementation((callback) => {
                  // Simulate the each callback being called with mockLeafGroup
                  callback.call(mockLeafGroup);
                }),
              }),
            }),
          }),
      ...
      // Verify that both left and right leaves are created
      expect(mockLeafGroup.append).toHaveBeenCalledWith('path');
      expect(mockLeafGroup.append).toHaveBeenCalledTimes(2); // Left and right leaves
    });
  });
});
