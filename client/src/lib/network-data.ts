...
export async function fetchNetworkDataById(artistId: string): Promise<NetworkData> {
  try {
    console.log(`🔍 [Frontend] Fetching network data by ID: "${artistId}"`);
    ...
    console.log(`✅ [Frontend] Received network data by ID with ${data.nodes?.length || 0} nodes`);
    return data;
  } catch (error) {
    console.error(`❌ [Frontend] Error fetching network data by ID:`, error);
    throw error;
  }
}
