...
export default function Home() {
  ...
  return (
    <div className={`relative w-full min-h-screen bg-black text-white main-container ${!showNetworkView ? 'overflow-x-hidden' : ''}`} style={{ pointerEvents: 'auto' }}>
      ...
      {!showNetworkView && (
        <div className="footer-content fixed bottom-0 left-0 right-0 text-center z-10 bg-gradient-to-t from-black/80 to-transparent" style={{ pointerEvents: 'auto', paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 160px)' }}>
      ...
      {!showNetworkView && (
        <div className="fixed left-1/2 transform -translate-x-1/2 z-50" style={{ bottom: 'calc(env(safe-area-inset-bottom, 16px) + 80px)' }}>
          <button
            onClick={() => {
