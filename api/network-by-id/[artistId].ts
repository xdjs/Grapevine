...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try {
      ...
      const prompt = `Search Spotify's "appears on" section and other collaboration data to find music industry professionals who have collaborated with ${artist.name}. Focus on producers, songwriters, and other artists who have worked with them.
...
- Prioritize collaborations found in Spotify's "appears on" section and official album credits`;
      ...
      // Detect roles for main artist first (evidence-based)
      ...
      try {
        ...
        const result = await roleService.computeRoles(artist.name, { includeArtistByDefault: true });
        const detected = (result.roles || []).filter(r => ['artist','producer','songwriter'].includes(r));
        if (detected.length > 0) {
          mainArtistRoles = detected;
          console.log(`✅ [Vercel] Evidence-based roles for MAIN artist "${artist.name}":`, mainArtistRoles);
        }
      } catch {
...
- Return ONLY the JSON object, no other text`;
      ...
      for (const collaborator of collaborationData.artists || []) {
        ...
        // Use evidence-based service to detect all roles for this artist node
